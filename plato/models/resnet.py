...
class Model(nn.Module):
    ...
    def forward(self, x):
        ...
        if not (Config().data.datasource == "FEMNIST"
                or Config().data.datasource == "MNIST"):
            out = F.avg_pool2d(out, 4)
        else:  # FEMNIST for ResNet [Model part]
    ...
    def get_model(model_type):
        ...
        if resnet_type == 18:
            result = Model(BasicBlock, [2, 2, 2, 2], num_classes)
        elif resnet_type == 34:
            result = Model(BasicBlock, [3, 4, 6, 3], num_classes)
        elif resnet_type == 50:
            result = Model(Bottleneck, [3, 4, 6, 3], num_classes)
        elif resnet_type == 101:
            result = Model(Bottleneck, [3, 4, 23, 3], num_classes)
        elif resnet_type == 152:
            result = Model(Bottleneck, [3, 8, 36, 3], num_classes)
        else:
            raise NotImplementedError

        # FEMNIST for ResNet [Model part]
        if Config().data.datasource == "FEMNIST" or Config().data.datasource == "MNIST":
