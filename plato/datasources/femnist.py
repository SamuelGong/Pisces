...
class CorruptedFEMNIST(Dataset):
    def __init__(self, orig, num_classes, seed):
        super(CorruptedFEMNIST, self).__init__()
        self.orig_femnist = orig
        np.random.seed(seed)
        self.num_classes = num_classes

    def __getitem__(self, index):
        x, y = self.orig_femnist[index]  # get the original item
        my_x = x
    ...
    def __len__(self):
        return self.orig_femnist.__len__()
...
class DataSource(base.DataSource):
    ...
    def __init__(self, client_id=0):
        ...
        if "resnet" in Config().trainer.model_name:  # FEMNIST for ResNet [Dataset part]
    ...
    def simulate_data_corruption(self, client_id):
        ...
        if hasattr(Config().data, "data_corruption") \
            ...
            self.trainset = CorruptedFEMNIST(self.trainset,
                                             num_classes,
                                             client_seed)
